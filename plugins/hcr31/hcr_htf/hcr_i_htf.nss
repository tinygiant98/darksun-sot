/// -----------------------------------------------------------------------------
/// @file:  hcr_i_htf.nss
/// @brief: HCR2 Hunger Thirst Fatigue System (core)
/// -----------------------------------------------------------------------------
...
const int HTF_EVENT_UPDATE_HUNGERTHIRST = 30000;
const int HTF_EVENT_UPDATE_FATIGUE = 30001;
const int HTF_EVENT_UPDATE_ALCOHOL = 30002;
const int HTF_EVENT_UPDATE_ALL = 30003;

// -----------------------------------------------------------------------------
//                              Function Prototypes
// -----------------------------------------------------------------------------

/// @brief Event script for module-level OnModuleLoad event.  Initializes
///     event and variables for htf system objects.
void hungerthirst_OnModuleLoad();

/// @brief Event script for module-level OnClientEnter event.  Initializes
///     the hunger and thirst system for the entering player character.
void hungerthirst_OnClientEnter();

/// @brief Event script for module-level OnClientEnter event.  Initializes
///     the fatigue system for the entering player character.
void fatigue_OnClientEnter();

/// @brief Event script for module-level OnPlayerDeath event.  Kills any active
///     hunger/thirst timers and deletes all subsystem variables.
void hungerthirst_OnPlayerDeath();

/// @brief Event script for module-level OnPlayerRestFinished event.  Resets
///     the drunk timer, if applied, and deletes variables associated with
///     intoxication.
void hungerthirst_OnPlayerRestFinished();

/// @brief Event script for module-level OnPlayerRestFinished event.  Removes
///     variable associated with fatigue and resets save counters.
void fatigue_OnPlayerRestFinished();

/// @brief Event script for local OnPlaceableUsed event.  This script should
///     be set on any placeable that is usable as a source of food or drink.
/// @note The placeable should be a usable non-container (no inventory).  Assign
///     variables to this placeable just as you would to any h2_fooditem.
void htf_OnPlaceableUsed();

/// @brief Event script for local OnTriggerEnter event.  This script should be
///     set as the OnEnter handler for any trigger/location that can act
///     as a source to fill a player's canteen.
void hungerthirst_OnTriggerEnter();

/// @brief Event script for local OnTriggerExit event.  This script should be
///     set as the OnExit handler for any trigger/location that can act
///     as a source to fill a player's canteen.
void hungerthirst_OnTriggerExit();

/// @brief Tag-based script for the player's canteen.
void htf_canteen();

/// @brief Tag-based script for any food item.
void htf_fooditem();

/// @brief Timer expiration script for intoxication effects.
void htf_drunk_OnTimerExpire();

/// @brief Timer expiration script for hunger thirst checks.
void htf_ht_OnTimerExpire();

/// @brief Timer expiration script for fatigue checks.
...
void h2_DisplayHTInfoBars(object oPC)
{
    ...
    if (H2_HTF_DISPLAY_INFO_BARS_ON_NUI)
        SignalEvent(oPC, EventUserDefined(HTF_EVENT_UPDATE_HUNGERTHIRST));
    else
    {
...
void h2_InitHungerThirstCheck(object oPC)
{
    ...
    int timerID = CreateTimer(oPC, H2_HT_ON_TIMER_EXPIRE, HoursToSeconds(1), 0, 0.0, CORE_HOOK_TIMERS);
...
void h2_ApplyAlchoholEffects(object oPC, object oItem)
{
    ...
    if (alcoholValue > 0.0)
    {
        ...
        {
            int timerID = CreateTimer(oPC, H2_HT_DRUNK_ON_TIMER_EXPIRE, 150.0, 0, 30.0, CORE_HOOK_TIMERS);
...
void h2_DisplayFatigueInfoBar(object oPC)
{
    ...
    if (H2_HTF_DISPLAY_INFO_BARS_ON_NUI)
        SignalEvent(oPC, EventUserDefined(HTF_EVENT_UPDATE_FATIGUE));
    else
    {
...
void h2_InitFatigueCheck(object oPC)
{
    ...
    int timerID = CreateTimer(oPC, H2_FATIGUE_ON_TIMER_EXPIRE, HoursToSeconds(1), 0, 0.0, CORE_HOOK_TIMERS);
    StartTimer(timerID, FALSE);
...
void htf_OnModuleLoad()
{
    NUI_DefineForms("hcr_f_htf");
}

void htf_OnCreatureUserDefined()
{
    NUI(OBJECT_SELF);
}
...
void hungerthirst_OnModuleLoad()
{
    object oTrigger = GetObjectByTag(H2_HT_TRIGGER);
    int n; while (GetIsObjectValid(oTrigger))
    {
        SetLocalString(oTrigger, TRIGGER_EVENT_ON_ENTER, "hungerthirst_OnTriggerEnter:only");
        SetLocalString(oTrigger, TRIGGER_EVENT_ON_EXIT, "hungerthirst_OnTriggerExit:only");
    }
}
