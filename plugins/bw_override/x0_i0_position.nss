...
string _LocationToString(location loc);

// Turn a vector into a string. Useful for debugging.
string _VectorToString(vector vec);
...
string _LocationToString(location loc)
{
    return "(" + GetTag(GetAreaFromLocation(loc)) + ")"
        + " " + _VectorToString(GetPositionFromLocation(loc))
...
location GetCenterPointOfArea(object oArea)
{
    ...
    //PrintString("Center vector: " + _VectorToString(vTmp));
