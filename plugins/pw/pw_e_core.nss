...
/// @todo none of these are really publc, only being used in this plugin, make private?
...
void pw_OnClientLeave()
{
    ...
    /// @todo instead of requiring this check, can we cancel the event when
    ///     LOGIN_BOOT is set?  Liekly not because LOGIN-BOOT is set in 
    ///     a different event, so add a check to event man that if LOGIN_BOOT
    ///     is true, cancel this event completely and let the pc be booted.
