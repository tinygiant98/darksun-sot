...
void DS_Module_Init()
{
    WriteTimestampedLogEntry("* Initializing Dark Sun: Sands of Time Module");

    WriteTimestampedLogEntry("* Increasing Instruction Limit");
...
*/
    ...
    for (nEvent = EVENT_SCRIPT_MODULE_ON_HEARTBEAT; nEvent <= EVENT_SCRIPT_MODULE_ON_PLAYER_TARGET; nEvent++)
    {
        ...
        string sNewScript = "hook_nwn";

        int bSet = SetEventScript(GetModule(), nEvent, sNewScript);

        if (!bSet)
            WriteTimestampedLogEntry("WARNING: Failed to set module event script for (" + sEvent + ")");
        else
            WriteTimestampedLogEntry("Set module event " + sEvent + " to " + sNewScript);
    }

    //Debug("* Resetting Instruction Limit");
    //NWNX_Util_SetInstructionLimit(-1);

    WriteTimestampedLogEntry("* Dark Sun: Sands of Time Module Initialization Complete!");
}
